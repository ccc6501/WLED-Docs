...
export function DevWorkbench() {
  ...
  async function computeEmbedding() {
    ...
    try {
      ...
      const buffer = await crypto.subtle.digest('SHA-256', encoder.encode(embeddingText));
      const view = new DataView(buffer);
      const vector: number[] = [];
      for (let i = 0; i < view.byteLength; i += 4) {
        vector.push(view.getInt32(i) / 2 ** 31);
      }
      setEmbeddingVector(vector.slice(0, 32));
    } catch {
      setEmbeddingVector([]);
    }
  }
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-border/60 bg-black/40 p-6 shadow-glass">
          ...
          <textarea
            ...
            rows={6}
            className="mt-4 w-full rounded-2xl border border-border/40 bg-black/30 p-4 text-sm outline-none focus:border-primary"
            placeholder="Paste text to hash into a vector"
          />
          <button
