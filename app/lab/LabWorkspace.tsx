...
export function LabWorkspace() {
  ...
  async function indexAll() {
    ...
    try {
      ...
      setIndexSummary(`Indexed ${data.indexed ?? 0} chunk(s) • mode: ${data.mode ?? 'unknown'}`);
      setIndexError(null);
    } finally {
      setIsIndexing(false);
    }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
      <div className="space-y-4">
        <div className="rounded-3xl border border-border/60 bg-black/40 p-6 shadow-glass">
          ...
          {indexError && <p className="mt-2 text-xs text-destructive">{indexError}</p>}
          {indexSummary && !indexError && <p className="mt-2 text-xs text-muted-foreground">{indexSummary}</p>}
